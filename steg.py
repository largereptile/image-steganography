...
def encode_from_temp(old_filename, png_filename, message):
...
def encode():
    if request.method == 'POST':
        ...
        old_filename = f"{random_name}{file_ext}"
        ...
        if file and allowed_file(file_ext):
            ...
            encode_from_temp(old_filename, png_filename, message)
...
def decode():
    if request.method == 'POST':
        ...
        if file and allowed_file(file_ext):
            file.save(filename)
            message = decode_from_temp(filename)
